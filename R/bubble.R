bubble = function(settings) {
  ...
  if (is.null(cex) || length(cex) != nrow(settings$datapoints)) {
    return(invisible())
  }
...
sanitize_bubble = function(settings) {
  env2env(
    settings,
    environment(),
    c("datapoints", "pch", "alpha", "bg", "cex", "bubble")
  )
  ...
  bubble_bg_alpha = if (
    !is.null(bg) && length(bg) == 1 && is.numeric(bg) && bg > 0 && bg <= 1
  ) {
    bg
  } else {
    1
  ...
  env2env(
    environment(),
    settings,
    c("datapoints", "bubble_pch", "bubble_alpha", "bubble_bg_alpha")
  )
}
