...
draw_legend = function(
  ...
  if (!dynmar) {
    restore_margin_inner(ooma, topmar_epsilon = topmar_epsilon)
  }
  ...
  if (outer_side) {
    ...
    if (outer_bottom) {
      ...
      if (
        has_sub && (is.null(.tpar[["side.sub"]]) || .tpar[["side.sub"]] == 1)
      ) {
...
tinylegend = function(
  ...
  if (gradient) {
    if (!more_than_n_unique(legend_args[["col"]], 1)) {
      if (
        !is.null(legend_args[["pt.bg"]]) &&
          length(legend_args[["pt.bg"]]) == 100
      ) {
