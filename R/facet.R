...
draw_facet_window = function(
  ...
  if (nfacets > 1) {
    ...
    if (dynmar) {
      if (par("las") %in% 1:2) {
        ...
        if (type == "ridge") {
          ...
          # yaxl = axTicks(2)
          yaxlabs = axisTicks(
            usr = extendrange(ylim, f = 0.04),
            log = par("ylog")
          )
        }
        if (!is.null(yaxl)) {
          yaxlabs = tinylabel(yaxlabs, yaxl)
        }
        # whtsbp = grconvertX(max(strwidth(yaxl, "figure")), from = "nfc", to = "lines") - 1
        whtsbp = grconvertX(
          max(strwidth(yaxlabs, "figure")),
          from = "nfc",
          to = "lines"
        ) -
          grconvertX(0, from = "nfc", to = "lines") -
          1
        if (whtsbp > 0) {
      ...
      if (par("las") %in% 2:3) {
        ...
        # xaxlabs = axTicks(1)
        xaxlabs = if (is.null(xlabs)) {
          axisTicks(usr = extendrange(xlim, f = 0.04), log = par("xlog"))
        } else if (!is.null(names(xlabs))) {
          names(xlabs)
        } else {
          xlabs
        }
        if (!is.null(xaxl)) {
          xaxlabs = tinylabel(xaxlabs, xaxl)
        }
        whtsbp = grconvertX(
          max(strwidth(xaxlabs, "figure")),
          from = "nfc",
          to = "lines"
        ) -
          1
    ...
    if (type == "spineplot") {
      omar[4] = 2.1
    } # FIXME catch for spineplot RHS axis labs
    if (par("las") %in% 1:2) {
      # extra whitespace bump on the y axis
      ## overrides for ridge and some types that use integer spacing with (named) axis labels ## FXIME
      if (type == "ridge") {
  ...
  for (ii in ifacet) {
    ...
    if (isTRUE(axes) || isTRUE(facet.args[["free"]])) {
      ...
      if (isTRUE(facet.args[["free"]])) {
        ...
        # individual facet.
        xfree = if (!is.null(facet)) {
          split(c(x, xmin, xmax), facet)[[ii]]
        } else {
          c(x, xmin, xmax)
        }
        yfree = if (!is.null(facet)) {
          split(c(y, ymin, ymax), facet)[[ii]]
        } else {
          c(y, ymin, ymax)
        }
        if (null_xlim) {
    ...
    if (!is.null(facet)) {
      ...
      if (xlog) {
        corners[1:2] = 10^(corners[1:2])
      }
      if (ylog) {
        corners[3:4] = 10^(corners[3:4])
      }
      ...
      if (isTRUE(attr(facet, "facet_grid"))) {
        ...
        if (ii %in% 1:nfacet_cols) {
          ...
          xpos = if (xlog) {
            10^(mean(log10(corners[1:2])))
          } else {
            mean(corners[1:2])
          }
          if (ylog) {
            ypos = grconvertY(0.4, from = "lines", to = "user") /
              grconvertY(0, from = "lines", to = "user")
            ypos = corners[4] * ypos
          } else {
            ypos = grconvertY(0.4, from = "lines", to = "user") -
        ...
        if (ii %% nfacet_cols == 0 || ii == nfacets) {
          ...
          if (xlog) {
            xpos = grconvertX(0.4, from = "lines", to = "user") /
              grconvertX(0, from = "lines", to = "user")
            xpos = corners[2] * xpos
          } else {
            xpos = grconvertX(0.4, from = "lines", to = "user") -
        ...
        if (isTRUE(facet_rect)) {
          line_height = (facet_title_lines + .1) * facet_text / cex_fct_adj
          if (ylog) {
            line_height = grconvertY(line_height, from = "lines", to = "user") /
              grconvertY(0, from = "lines", to = "user")
            rect_height = corners[4] * line_height
          } else {
            line_height = grconvertY(line_height, from = "lines", to = "user") -
    ...
    # panel grid lines
    if (is.null(grid)) {
      grid = get_tpar("grid", tpar_list = tpars)
    }
    if (!is.null(grid)) {
      if (is.logical(grid)) {
        ...
        if (isTRUE(grid)) {
          ...
          if (!is.null(xaxb)) {
            abline(
              v = xaxb,
              col = get_tpar("grid.col", tpar_list = tpars),
              lty = get_tpar("grid.lty", tpar_list = tpars),
              lwd = get_tpar("grid.lwd", tpar_list = tpars)
            )
            gnx = NA
          } else if (!any(c(par("xlog"), type == "boxplot"))) {
            xg = if (!inherits(x, c("POSIXt", "Date"))) {
              axTicks(side = 1)
            } else {
              axTicksDateTime(side = 1, x = x)
            }
            abline(
              v = xg,
              col = get_tpar("grid.col", tpar_list = tpars),
          ...
          if (!is.null(yaxb)) {
            ...
            yg = if (!inherits(y, c("POSIXt", "Date"))) {
              axTicks(side = 2)
            } else {
              axTicksDateTime(side = 2, x = x)
            }
            abline(
              h = yg,
              col = get_tpar("grid.col", tpar_list = tpars),
...
get_facet_fml = function(formula, data = NULL) {
  ...
  if (!is.null(data)) {
    m = m[c(
      1L,
      match(
        c("formula", "data", "subset", "na.action", "drop.unused.levels"),
        names(m),
        0L
      )
    )]
  ...
  if (NCOL(mf) < xfacet_loc) {
    stop("formula should specify at least one variable on the right-hand side")
  }
