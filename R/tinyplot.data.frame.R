...
#'   drawn instead.
#' @param by (pairs-case only) optional grouping variable. Either 1) a character
...
#'   suppressed.
#' @param labs (pairs-case only) logical indicating whether the axes labels
#'   (titles) for each sub-plot in the pairs-style case should be shown. Default
#'   is `FALSE`.
#' @param frames (pairs-case only) logical indicating whether each sub-plot should
...
#'
#' ## tinypairs can also create 2x2 pairs display
#' tinypairs(iris[, c(3, 1)], by = iris$Species)
#'
...
tinyplot.data.frame = function (x, formula = NULL, by = NULL, labs = FALSE, frames = FALSE, ...) {
  ...
  ## behavior without formula:
  ## 1d: y ~ 1
  ## 2d: y ~ x
  ## 3d: pairs
  if (is.null(formula) && n > 2L) {
    cl[[1L]] = quote(tinyplot::tinypairs)  
  } else {
...
#' @rdname tinyplot.data.frame
#' @export
tinypairs = function (x, by = NULL, labs = FALSE, frames = FALSE, ...) {
  ## original call
  cl = match.call()
