...
#' * `xaxr`, `yaxr`: Numeric giving the rotation of the x- and y-axis tick labels, in degrees counter-clockwise. `NULL` (the default) leaves the labels flat, under the control of `las`. Unlike `las`, which is limited to the four right angles, any angle is permitted; tinyplot draws the labels itself whenever one is set. Setting one overrides `las` for that axis alone, leaving the other axis under `las` as usual. Sets the default for the `xaxr` and `yaxr` arguments of [`tinyplot()`], which take precedence.
...
assert_tpar = function(.tpar) {
  ...
  assert_numeric(.tpar[["xaxr"]], len = 1, lower = -360, upper = 360, null.ok = TRUE, name = "xaxr")
  assert_numeric(.tpar[["yaxr"]], len = 1, lower = -360, upper = 360, null.ok = TRUE, name = "yaxr")
