...
data_spineplot = function(
  ...
  fun = function(settings, ...) {
    ...
    if (is.factor(datapoints$x)) {
      ...
      if (is.null(breaks)) {
        breaks = if (!is.null(xaxb)) {
          xaxb
        } else if (is.null(weights)) {
          nclass.Sturges(x)
        } else {
          ceiling(log2(sum(weights)) + 1)
        }
      }
      ...
      if (length(breaks) == 1L) {
        ...
        rg = if (is.null(weights)) {
          range(x, na.rm = TRUE)
        } else {
          range(x[weights > 0], na.rm = TRUE)
        }
    ...
    if (!is.null(ylevels)) {
      ylevels = if (is.numeric(ylevels)) {
        levels(datapoints$y)[ylevels]
      } else {
        ylevels
      }
      if (anyNA(ylevels) || !all(ylevels %in% levels(datapoints$y))) {
    ...
    datapoints = Map(
      function(dat, x.categorical, off) {
        ...
        yat = rbind(
          0,
          apply(proportions(tab[, ny:1L, drop = FALSE], 1L), 1L, cumsum)
        )
        yat[is.na(yat)] = 1
    ...
    # catch for x_by / y/by
    if (isTRUE(x_by)) {
      datapoints$by = factor(rep(xaxlabels, each = ny))
    } # each x label extends over ny rows
    if (isTRUE(y_by)) {
...
draw_spineplot = function(
  ...
  fun = function(
    ...
    if (is.null(xaxlabels)) {
      xaxlabels = type_info[["xaxlabels"]]
    }
    if (is.null(yaxlabels)) {
      yaxlabels = type_info[["yaxlabels"]]
    }
    ...
    if (
      type_info[["xaxt"]] %in%
        ...
        !all(c(type_info[["xaxt"]], type_info[["yaxt"]]) == "n")
    ) {
      ilwd = 0
    }
  ...
  if (type == "none") {
    ...
    if (type == "labels") {
      ...
      if (!("lwd.ticks" %in% names(args))) {
        args$lwd.ticks = if (categorical) 0 else 1
      }
    } else if (type == "axis") {
      if (categorical) {
        args$tick = FALSE
...
to_hcl = function(x) {
  ...
  x = cbind(
    H = atan2(x[, 3L], x[, 2L]) * 180 / pi,
    C = sqrt(x[, 2L]^2 + x[, 3L]^2),
    L = x[, 1L]
  )
  x[is.na(x[, 1L]), 1L] = 0
...
seq_palette = function(x, n, power = 1.5) {
  ...
  hcl(
    h = x[1L],
    c = seq.int(from = x[2L]^(1 / power), to = 0, length.out = n + 1)[
      1L:n
    ]^power,
    l = 100 -
      seq.int(
        from = (100 - x[3L])^(1 / power),
