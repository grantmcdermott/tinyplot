...
::: {.fragment}
Did we cover over any points? Um, maybe check that later...
:::
...
2. Bring feature parity against grid-based 📦s like **ggplot2** and **lattice**.   
:::
...
::: {.fragment}
The `"!"` suffix places the legend _outside_ the plot area.
:::
...
All tinyplot types can be passed as either:

- string: `"p"`, `"density`, `"lm"`, etc.
- function: `type_points()`, `"type_density()"`, `type_lm()`, etc.
...
- Remember: `tinytheme(...)` is persistent, which is why we've had the "clean" theme enacted for most of this talk.
- To revert back to the **graphics** default, call `tinytheme()` with no args.
...
tinytheme(
  "ipsum",
  pch = 19,
  cex = 1.2, cex.main = 2, cex.sub = 1.5, cex.lab = 1.5,
