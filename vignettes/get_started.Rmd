...
grouping, and also add the original values to our model predictions. Note that
for this particular example, we'll use the `facet = "by"` convenience shorthand
to facet along the same month variable as the colour grouping. But you can
easily specify different `by` and `facet` variables if that's what your data
support.
