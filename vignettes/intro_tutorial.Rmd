...
The examples that we have seen thus far are all based on discrete grouping
variables. However, please note that **tinyplot** also supports continuous
grouping variables, which automatically yield gradient legends.
...
`"ribbon"` type arguments. An canonical use-case is regression analysis and
prediction.
...
is expensive (e.g., propoduction piplines, continuous integration or an R
