...
variables. The exact behaviour depends on how many unique groups are detected; 
see `?tpar(legend.ugc)`. But it should generally do what you expect, and can be
further customized in a similar vein to the discrete legends that we have seen
so far (keyword positioning, alpha transparency, etc.)

```{r legend_gradient}
tinyplot(Temp ~ Day | Temp, data = aq, palette = "inferno")
```
...
changing global parameters (via `par`) or invoking `tinyplot` arguments. Here's a
