...
ensure that each individual facet is drawn on correctly. Second, you can
...
drawing/annotating functions. You can combine multiple drawing functions by
wrapping them with curly brackets`{}`, and even pass `tinyplot()` back towards
itself.

Here's a slightly more complicated density plot example, where we use
`draw = {...}` to pass multiple drawing functions, including plotting all of the
distributions in the background of each facet via a secondary `tinyplot()` call.

```{r draw_complex}
palette("tableau")
tinyplot(
  ~Temp | Month, aq, type = "density", facet = "by", lwd = 2, fill = "by",
  frame = FALSE,
  draw = {
    tinyplot(~Temp | Month, aq, type = "density", col = "grey", fill = "grey90", add = TRUE)
    abline(v = 75, lty = 2)
    text(75, 0.085, "Cold", pos = 2)
    text(75, 0.085, "Hot", pos = 4)
  }
)
```
