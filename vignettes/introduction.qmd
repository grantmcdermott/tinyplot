...
The previous examples hint at a core part of the **tinyplot** philosophy: it
should be easy to customize your plot appearance. Take colours, for instance.
The default set of discrete colours for grouped plots are inherited from the
user's current global palette. (Most likely the "R4" set of colors; see
`?palette`). But this behaviour is easily customized via the `palette` argument,
which accepts any of built-in R palettes as a convenience string.^[Note that
case-insensitive and partial matching is allowed. See `palette.pals()` and
`hcl.pals()` for the full set of available palettes. Or, read Zeileis & Murrell
(2023, The R Journal,
[doi:10.32614/RJ-2023-071](https://doi.org/10.32614/RJ-2023-071)).]
For example:
...
line-based plots with x-sorted data, and pairs well with dynamic themes (like we
enable above) for tighter margins.
...
if there is a high degree of overlap. A good alternative in such cases are
ridge(line) plots, which **tinyplot** supports out of the box. Note that ridge
plots are best viewed with their own dedicated theme.
...
change the background fill of facet strip fill.

```{r facet_nrow}
tinyplot(
  Temp ~ Day, aq,
  facet = ~Month,
  facet.args = list(nrow = 1, bg = "lavender"),
  type = "lm",
  main = "Predicted air temperatures"
)
```
...
Don't forget to reset the theme.
...
the [Plot types](types.qmd) and [Themes](themes.qmd) vignettes. Or, simply take
a look at our [Gallery](gallery.qmd) page if you're looking for inspiration and
code examples.
