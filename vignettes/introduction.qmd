...
For convenience, the package also includes a `tinyplot_add()` function which captures the last `tinyplot()` call, keeps all the same arguments, and modifies just the arguments that the user explicitly wants to change. 
