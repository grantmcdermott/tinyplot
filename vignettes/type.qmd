...
```{r}
#| layout-ncol: 2
tinyplot(Temp ~ Month, data = aq, main = "Default jittering", 
    type = "jitter")

tinyplot(Temp ~ Month, data = aq, main = "Modest jittering",
...
```{r}
#| layout-ncol: 2
tinyplot(Hot ~ Wind, data = aq, main = "Logit with 95% interval",
    type = type_glm(family = "binomial"))

tinyplot(Hot ~ Wind, data = aq, main = "Logit with 50% interval",
...
```{r}
tinyplot(Temp ~ Wind | Month, data = aq, type = type_lm(se = FALSE))
```
