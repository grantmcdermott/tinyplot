...
1. `data_*()`: Function factory.
    - Accepts `...` and a named list called `settings` which holds many internal objects
        - Inputs must include `...`
        - `settings$datapoints` Is the most important object. It is a data frame with the datapoints to plot. 
        - Other objects that can be modified by `data_*()` include: `settings$by`, `settings$facet`, `settings$ylab`, `settings$palette`
    - Returns a named list with modified versions of the `settings` values.
